...
/datum/sex_panel_action/other/penis/hemi/anal_double/get_start_message(user, target)
	var/pose_state = get_pose_key(user, target)
	return "[user] [get_pose_text(pose_state)] прижимает оба ствола к анусу [target]."

/datum/sex_panel_action/other/penis/hemi/anal_double/get_perform_message(user, target)
	var/pose_state = get_pose_key(user, target)
...
/datum/sex_panel_action/other/penis/hemi/anal_double/handle_climax_message(mob/living/carbon/human/user, mob/living/carbon/human/target)
	var/list/result = ..()
	if(!islist(result))
		result = list(result)
