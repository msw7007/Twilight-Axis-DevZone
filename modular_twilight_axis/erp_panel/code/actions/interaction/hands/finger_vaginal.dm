...
/datum/sex_panel_action/other/hands/finger_vaginal/get_perform_message(mob/living/carbon/human/user, mob/living/carbon/human/target)
	var/pose_state = get_pose_key(user, target)
	var/message = "[user] [get_pose_text(pose_state)], [get_force_text()] и [get_speed_text()] проникает пальчиков в киску [target]."
...
/datum/sex_panel_action/other/hands/finger_vaginal/on_perform(mob/living/carbon/human/user, mob/living/carbon/human/target)
	. = ..()
